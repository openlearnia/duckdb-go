...
func TestGrainDuckLakeExtensionLoad(t *testing.T) {
	...
	// allow_unsigned_extensions is a startup-only setting, so it goes through
	// the DSN config; signed CI-published extensions still load under it.
	connector, err := NewConnector(":memory:?allow_unsigned_extensions=true", nil)
	if err != nil {
		t.Fatal(err)
	}
